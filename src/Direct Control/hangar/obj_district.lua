...
---Triggers when the district gets selected.
---@param q game_obj_district
function picked_true(q)
	if (q.my_number == 5) then
		local localizedString = { LocalizedDefaultValue = "Provides continuous communication with the mechs on a mission, and also quickly broadcasts messages throughout the city.## Improvement will allow more mechs to be send on missions" }
		q.district_desc = Mod.Common.GetLocalizedString("Custom", "DistrictDescription5", localizedString)
	end
end

---Triggers when the district gets unselected.
---@param q game_obj_district
function picked_false(q)
end

---district was just upgraded
---@param q game_obj_district
function upgraded(q)
end

---assault team was assigned to this district
---@param q game_obj_district
function team_inside(q)
end