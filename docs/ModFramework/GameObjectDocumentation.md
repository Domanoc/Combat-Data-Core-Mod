...
One important not is that the game uses gamemaker structs and ds maps. both are seen in lua as numbers but can be interacted with differently.
...
### DS MAP
Is a reference handle to the gamemaker ds_map data structure, stores key and value pairs.
...
ds_grid_create, ds_grid_write, ds_grid_read, ds_grid_set, ds_grid_get, ds_map_exists, ds_map_create, ds_map_destroy, ds_map_add, ds_map_copy, ds_map_find_value
